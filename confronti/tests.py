...
class ConfrontoViewTests(TestCase):
    ...
    def test_archive_page_requires_admin_even_for_operator(self):
        self.login_operator()
        response = self.client.get("/archivio-report/")
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response["Location"], "/")

    def test_operator_can_archive_report_without_accessing_archive_dashboard(self):
        self.login_operator()
    ...
    def test_archive_page_lists_and_updates_saved_reports(self):
        ...
        self.assertContains(response, "Nome cliente:")
        self.assertContains(response, "Data esecuzione:")
        self.assertContains(response, "Fornitura:")
        self.assertContains(response, "POD / PDR:")
        ...
        response = self.client.get(f"/archivio-report/cartella/{folder.pk}/?report_id={report.pk}")
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Dettaglio confronto")
        ...
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response["Location"], f"/archivio-report/cartella/{folder.pk}/?report_id={report.pk}")
        report.refresh_from_db()
    ...
    def test_archive_page_can_build_summary_from_selected_reports(self):
        ...
        self.assertEqual(len(self.client.session["last_report_summary"]["rows"]), 1)
        self.assertEqual(self.client.session["last_report_summary"]["rows"][0]["Indirizzo fornitura"], "Via Roma 10")
    ...
    def test_customer_area_always_calculates_with_illumia_only(self):
        ...
        self.assertNotIn("Scelta offerta Illumia", html)
        self.assertNotIn("Offerta variabile Illumia", html)
        self.assertNotIn("Offerta fissa Illumia", html)
